...
class PropertiesDesign(context: Context) : Design<PropertiesDesign.Request>(context) {
    ...
    fun inputUrl() {
        ...
        launch {
            ...
            if (url != profile.source) {
                val interval = if (Pinxixi.isSubscriptionUrl(url)) {
                    Pinxixi.defaultIntervalMs(url)
                } else {
                    profile.interval
                }
                profile = profile.copy(source = url, interval = interval)
            }
        }
    }
    ...
    fun inputInterval() {
        launch {
            ...
            val minMinutes = Pinxixi.minIntervalMinutes(profile.source)

            minutes = context.requestModelTextInput(
                initial = if (minutes == 0L) {
                    if (Pinxixi.isSubscriptionUrl(profile.source)) Pinxixi.DEFAULT_INTERVAL_MINUTES.toString() else ""
                } else {
                    minutes.toString()
                },
                title = context.getText(R.string.auto_update),
                hint = context.getText(R.string.auto_update_minutes),
                error = context.getString(R.string.at_least_15_minutes).replace("15", minMinutes.toString()),
                validator = validatorAutoUpdateInterval(profile.source)
            ).toLongOrNull() ?: 0
    ...
    private fun ModelProgressBarConfigure.applyFrom(status: FetchStatus) {
        when (status.action) {
            ...
            FetchStatus.Action.VerifyingConfiguration -> {
